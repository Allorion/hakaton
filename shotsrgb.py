...
import time
import ctypes
from pic_cols.hakaton.test import colorzRGB
...
def compare(rgbs):
    ...
    if len(rgbs) >= 3:
        rgbs.pop(1)
    if (compareOne(rgbs[1][0], rgbs[0][0]) and compareOne(rgbs[1][1], rgbs[0][1]) and compareOne(rgbs[1][2], rgbs[0][2])):
        return True
    else:
        return False
